...
package com.finalteam.galleryfinal.widget;
...
public class FloatingActionButton extends ImageButton {
    ...
    void init(Context context, AttributeSet attributeSet) {
        TypedArray attr = context.obtainStyledAttributes(attributeSet, cn.finalteam.galleryfinal.R.styleable.GFFloatingActionButton, 0, 0);
        mColorNormal = attr.getColor(cn.finalteam.galleryfinal.R.styleable.GFFloatingActionButton_fabColorNormal, Color.BLACK);
        mColorPressed = attr.getColor(cn.finalteam.galleryfinal.R.styleable.GFFloatingActionButton_fabColorPressed, Color.BLACK);
        mIcon = attr.getResourceId(cn.finalteam.galleryfinal.R.styleable.GFFloatingActionButton_fabIcon, 0);
        mTitle = attr.getString(cn.finalteam.galleryfinal.R.styleable.GFFloatingActionButton_fabTitle);
        attr.recycle();

        updateCircleSize();
        mShadowRadius = getDimension(cn.finalteam.galleryfinal.R.dimen.fab_shadow_radius);
        mShadowOffset = getDimension(cn.finalteam.galleryfinal.R.dimen.fab_shadow_offset);
        updateDrawableSize();
    ...
    private void updateCircleSize() {
        mCircleSize = getDimension(cn.finalteam.galleryfinal.R.dimen.fab_size_normal);
    }
    ...
    public void setTitle(String title) {
        mTitle = title;
        TextView label = (TextView) getTag(cn.finalteam.galleryfinal.R.id.fab_label);
        if (label != null)
            label.setText(title);
    }
    ...
    void updateBackground() {
        ...
        float iconOffset = (mCircleSize - getDimension(cn.finalteam.galleryfinal.R.dimen.fab_icon_size)) / 2f;
