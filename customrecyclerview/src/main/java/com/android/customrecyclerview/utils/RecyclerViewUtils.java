...
import com.android.customrecyclerview.touchHelper.ItemTouchHelperFactory;
...
public class RecyclerViewUtils {
    ...
    private final class MyOnItemTouchListener implements RecyclerView.OnItemTouchListener {
        ...
        public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                // dTODO: 16/2/3 If we do not judge it,when we slide the banner,it will receive the
    ...
    public void setHeaderView(RecyclerView mRecycler, View header){
        ...
        if(mAdapter == null || !(mAdapter instanceof CustomRecyclerAdapter)){
            return ;
        }

        CustomRecyclerAdapter realAdapter = (CustomRecyclerAdapter)mAdapter;
    ...
    public  void prepareForDragAndSwipe(RecyclerView mRecycler, boolean shouldDrag, boolean shouldSwipe){
        if(mRecycler != null){
            if(!(mRecycler.getAdapter() instanceof CustomRecyclerAdapter)){
                return;
            }
